...
| ④ | **按验收包自验**；明示通过 → 关版 | 交「怎么验 + 验什么」；工程证伪为旁证；**不得**替人关版；通过后搬入 `docs/specs/archive/<id>/` |
...
7. **停。** 人关版后：`acceptance-passed` + 搬入 `docs/specs/archive/<id>/`；询问是否上线；得本轮「发布/上线」+ P4 再走 Skill `deploy`。不得自动上生产。
