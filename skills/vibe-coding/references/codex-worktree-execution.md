...
> 仅当 执行步骤 的 Workspace Strategy 选择 `codex-worktree` 或 `git-worktree` 时读取。
...
- 当前脏改动是否属于 执行步骤；
- 恢复指针、执行步骤、单一 owner 和所需外部资源 Claim 已存在；
...
不根据文档名称猜测工具存在。用户任务与 Subagent 是不同 owner；一个 执行步骤 绑定一个 owner
和一个 Workspace。执行顺序：

1. 为 执行步骤 建立机器 恢复指针，在 handoff 登记 objective、source、stop condition 和推荐
   Workspace；
...
4. 父任务把 `$vibe-coding`、objective 和 source 直接传给 Subagent；PM 不输入 恢复指针 路径；
...
8. 执行一个 执行步骤，并在该 Worktree 内取得 执行步骤 Evidence；
9. Delivery 需要分支时，按当前 surface 或 Git 能力创建唯一分支；
10. 同一用户任务需要在 Local 与其关联 Worktree 间移动且原生 Handoff 可用时使用
    Handoff；跨任务或 Handoff 不可用时停止该 owner，由项目 handoff 索引和 恢复指针 在目标
...
- 一个并行 执行步骤 对应一个 owner、恢复指针、Workspace、外部资源 Claim 集合和证据集合；
- 执行步骤/file 互斥由单 owner 与 Worktree 承担，不创建 执行步骤/file Claim；
- Worktree 不隔离数据库、端口、测试账号、浏览器或生产环境；
- 公共合同先行时，依赖 执行步骤 在同步新 base 后重新执行直接验证；
...
Claims、执行步骤 Status 和下一动作。恢复指针 是机器恢复指针，不作为 PM 操作说明。
