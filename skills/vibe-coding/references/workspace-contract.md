...
Route / 项目 handoff 索引负责跨任务、跨 surface 的持久恢复。
...
一个会修改文件的 Task 同一时刻只有一个 owner：
`current-chat | user-thread | subagent`。该
owner 只写一个 Local checkout 或一个 Worktree 中的 Task 边界。Task/file 所有权不写入
Claim 账本；由原生执行上下文、Workspace、依赖和 handoff 共同表达。

有原生 Subagent 时，只有 Task 能独立实现、验证和交接才分派；用户可见的新任务只在用户
明确要求时由 Codex 创建。没有这些能力或授权时，在当前 Build owner 串行执行，或由用户
用自然语言在新任务中启动并让 Codex 从 Route 恢复。
...
`claims` 只列数据库、端口、账号、浏览器、部署槽位等外部共享资源；不得为 Task、文件、
目录或代码合同创建 Claim。
...
- 使用受控外部共享资源前必须领取 resource Claim；创建 Worktree 本身不需要 Task/file
  Claim；
...
状态；跨任务恢复通过机器 Route 和项目 handoff 索引完成。项目 handoff 记录 Spec、Task、
Rail、原生 owner ID、Workspace、Branch、
PR、依赖、外部共享资源、状态和下一动作，但不复制聊天记录。PM 不需要输入 Route 路径、
线程 ID 或 Worktree 目录。
